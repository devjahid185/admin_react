...
export default function MarketplacePage({ token }) {
  ...
  const bulkDelete = async () => {
    ...
    setError("");
    try {
      await Promise.all(ids.map((id) => apiRequest(`/admin/resources/marketplace/${id}`, { method: "DELETE", token })));
              ...
              <tr key={item.id} className="border-t border-[#edf1f6]">
                <td className="px-3 py-2 md:px-4">
                  <input
                    ...
                    onChange={(e) => setSelectedIds((prev) => toggleSelectedId(prev, item.id, e.target.checked))}
