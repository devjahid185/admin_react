...
export default function WorkersPage({ token }) {
  ...
  const bulkDelete = async () => {
    ...
    setError("");
    try {
      await Promise.all(ids.map((id) => apiRequest(`/admin/resources/workers/${id}`, { method: "DELETE", token })));
              ...
              <tr key={w.id} className="border-t border-[#edf1f6]">
                <td className="px-3 py-2 md:px-4">
                  <input
                    ...
                    onChange={(e) => setSelectedIds((prev) => toggleSelectedId(prev, w.id, e.target.checked))}
                    aria-label={`Select record ${w.id}`}
                  />
                </td>
