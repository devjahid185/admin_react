...
export default function MessagesPage({ token }) {
  ...
  const bulkDelete = async () => {
    ...
    setError("");
    try {
      await Promise.all(ids.map((id) => apiRequest(`/admin/resources/messages/${id}`, { method: "DELETE", token })));
              ...
              <tr key={msg.id} className="border-t border-[#edf1f6]">
                <td className="px-3 py-2 md:px-4">
                  <input
                    ...
                    onChange={(e) => setSelectedIds((prev) => toggleSelectedId(prev, msg.id, e.target.checked))}
                    aria-label={`Select record ${msg.id}`}
                  />
                </td>
