...
export default function FaqsPage({ token }) {
  ...
  const bulkDelete = async () => {
    ...
    setError("");
    try {
      await Promise.all(ids.map((id) => apiRequest(`/admin/resources/faqs/${id}`, { method: "DELETE", token })));
              ...
              <tr key={faq.id} className="border-t border-[#edf1f6]">
                <td className="px-3 py-2 md:px-4">
                  <input
                    ...
                    onChange={(e) => setSelectedIds((prev) => toggleSelectedId(prev, faq.id, e.target.checked))}
                    aria-label={`Select record ${faq.id}`}
                  />
                </td>
