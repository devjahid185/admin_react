...
export default function BusinessesPage({ token }) {
  ...
  const bulkDelete = async () => {
    ...
    setError("");
    try {
      await Promise.all(ids.map((id) => apiRequest(`/admin/resources/businesses/${id}`, { method: "DELETE", token })));
              ...
              <tr key={b.id} className="border-t border-[#edf1f6]">
                <td className="px-3 py-2 md:px-4">
                  <input
                    ...
                    onChange={(e) => setSelectedIds((prev) => toggleSelectedId(prev, b.id, e.target.checked))}
                    aria-label={`Select record ${b.id}`}
                  />
                </td>
